...
1. DB 마이그레이션
   - [ ] `characters` / `story_characters` 테이블 생성
   - [ ] 기본 캐릭터 10종(seed) 데이터 삽입(이름, 성격, 말버릇, 프롬프트 키워드 등)

2. 백엔드
   - [ ] Character 엔티티/레포/서비스/컨트롤러 추가 (`GET /api/public/characters`)
   - [ ] Story 생성 요청에 `characterIds`(최대 2개) 처리 및 검증
   - [ ] Story 조회 DTO에 캐릭터 정보 포함, 캐릭터 이미지 응답/서빙 처리

3. 프론트엔드
   - [ ] `/stories/new` 페이지에 캐릭터 카드 UI 추가 (최대 2 선택)
   - [ ] Story 생성 시 선택 캐릭터 ID 전송 및 선택 상태 시각화
   - [ ] 동화 상세 등에서 선택된 캐릭터 정보 표시

4. AI 프롬프트 연계
   - [ ] 텍스트 생성 프롬프트에 캐릭터 프로필/말버릇 반영
   - [ ] 이미지 생성 시 캐릭터 설명 및 샘플 이미지를 이용해 스타일/디자인 유지

---

# 하트 결제 & 지갑 TODO

- [x] Mock PG 기반 하트 상품/주문/지갑 백엔드 도입
- [x] 프론트 결제 관리 페이지 및 잔액/사용 UX 연동
- [x] 동화 생성 시 하트 차감 및 부족 시 결제 안내 적용
- [ ] 실제 PG 연동 어댑터 구현 및 검증
- [ ] 어드민용 하트 상품/주문 관리 UI 및 RBAC 강화
- [ ] 하트 트랜잭션 모니터링 및 이상 거래 알림 설계
